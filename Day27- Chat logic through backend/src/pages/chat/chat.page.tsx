...
import chatServcie from "./chat.servcie";
    ...
    return (<>
        <div className="flex pe-5 mb-5 mt-3 shadow-lg bg-gray-100 hover:bg-blue-50 hover:cursor-pointer">
            <div className="w-1/4 mx-3">
                <img 
                src={user.image ? import.meta.env.VITE_IMAGE_URL+"user/"+user.image : "https://placehold.co/600x600"} className="rounded-full" alt="" />
            </div>
...
const ChatListView = () => {
    ...
    const loadAllUsers =async () => {
        try {
            const response= await chatServcie.getRequest("/chat/chat-list",{auth:true}) 
